...
@RestController
@RequestMapping(value = "/api/dt")
public class ApiZqDtController extends AbaseBusiness {
    final static String phoneRegex = "(1|861)(3|5|8)\\d{9}$*";
    final static String regex = "^[0][1-9]{2,3}-[0-9]{5,10}$";
    final static String dtregex = "^\\d{4}\\-\\d{1,2}\\-\\d{1,2}$";

    final static SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
    final static String agent = "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:57.0) Gecko/20100101 Firefox/57.0";
    ...
    @GetMapping("/get")
    public EntityResult get(String q, int pages, int pageIndex, HttpServletResponse response) {
        ...
        while (pageIndex < pages + 1) {
            ...
            System.out.println("关键词：" + q + "，第" + String.valueOf(mPageIndex) + "页，共" + String.valueOf(pages) + "页。");
            pageIndex++;
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        Thread.sleep(1500);
                        Document document = Jsoup.connect("https://s.weibo.com/user?q=" + q + "&auth=org_vip&page=" + mPageIndex)
                                .timeout(10000)
                                .userAgent(agent)
                        ...
                        for (int i = 0; i < elements.size(); i++) {
                            ...
                            try {
                                ...
                                if (date.isPresent()) {
                                    dates = dateFormat.format(date.get());
                                }
...
//                        System.out.println(dtmt.html());
        ...
        try {
            ...
            response.setHeader("Content-Disposition"
                    , "attachment;filename="
                            + URLEncoder.encode(q + "（" + (pageIndex - 1) + "-" + pages + "）数据导出("
